// https://editor.p5js.org/jht9629-nyu/sketches/i2P_11fWZ
// pixel-scope v2
// Center pixel of the video capture and display pixel RGB components.
// Touch on a pixel grid to see rgb components as numbers.
// Check box buttons Scan and Record to scan and record pixel grid below the video.

let my = { };

function setup() {
  my.version = '?v=2'; // update to verify change on mobile
  my.vwidth = 120; // Aspect ratio of video capture
  my.vheight = 160;
  my.vscale = 4; // scale up factor to canvas size
  my.cscale = 64; // scale down from video size to cross hair length
  my.colorSpanN = 16; // number of color spans in a row
  my.facingMode = 'user'; // user environment
  my.face = 1;
  my.scan = 0; // scan the cross hairs
  my.scanRate = 10; // scan step rate; bigger for slower
  my.record = 0; // record every n frames
  my.scanMargin = 0.0; // 0.25 // inset for scan
  my.scrolling = 1; // scroll to show last bottom element
  my.frame = 1;
