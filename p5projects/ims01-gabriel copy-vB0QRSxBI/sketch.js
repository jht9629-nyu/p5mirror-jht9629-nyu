// https://editor.p5js.org/gg447062/sketches/dpUAtjDYr
...
function setup() {
  ...
  for (let i = 0; i < palette.length; i++) {
    ...
    let thetaStep = (theta2 - theta1) / 10;
  ...
  if (path.length < M * N - 1) {
    ...
    for (let neigh of eventualNeighbors) {
      ...
      if (
        ...
        count < 2 &&
        !cursedCorners(projectedPath, M, N)
      ) {
...
function drawPath(path, x0, y0, m, n, ratio) {
  ...
  for (let k = 0; k < path.length - 1; k++) {
    ...
    let c1 = getCircle(p1[0], p1[1], ratio),
      c2 = getCircle(p2[0], p2[1], ratio);
    if (p1[1] == p2[1]) {
      myLine(c1, c2);
    } else {
      let theta1 = atan2(c1.y, c1.x);
      let theta2 =
        p1[1] < p2[1] ? theta1 + (2 * PI) / N : theta1 - (2 * PI) / N;
