// https://editor.p5js.org/jht9629-nyu/sketches/cc0BpWomv
// ims02-Vi v1

// How to select color range?
// How to control from non-mouse?
...
function setup() {
  ...
  createCanvas(my.len_start, my.len_start, WEBGL);
  frameRate(25);
...
function draw() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (let theta_deg = 0.5; theta_deg < 180; theta_deg += 0.5) {
      ...
      let x = radius * sin(radians(theta_deg)) * cos(radians(phi_deg));
      let y = radius * sin(radians(theta_deg)) * sin(radians(phi_deg));
      let z = radius * cos(radians(theta_deg));
...
function init_dim() {
  radius = height * (my.radius_start / my.len_start )
}
...
// https://editor.p5js.org/ViDolan/sketches/ugJpz97jW
// ims02-Vi
